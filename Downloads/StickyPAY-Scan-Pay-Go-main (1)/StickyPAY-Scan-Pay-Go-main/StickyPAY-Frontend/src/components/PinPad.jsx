...
export default function PinPad({ title, onSuccess, onCancel, isSetup }) {
    ...
    return (
        <div className="fixed inset-0 z-[100] flex items-center justify-center bg-black/80 backdrop-blur-sm">
            <div className="bg-gray-900 rounded-3xl p-6 w-80 border border-gray-700 shadow-2xl">
                <div className="flex justify-between items-center mb-4">
                    <h3 className="font-bold text-white text-lg">{title}</h3>
                    <button onClick={onCancel} className="w-8 h-8 rounded-full bg-gray-800 flex items-center justify-center text-gray-400 hover:text-white transition-colors">✕</button>
                </div>
                <p className="text-center text-gray-400 text-sm mb-6">{label}</p>
                <div className="flex justify-center gap-4 mb-8">
                    {[0, 1, 2, 3].map(i => (
                        <div key={i} className={`w-4 h-4 rounded-full border-2 transition-all ${current.length > i ? 'bg-yellow-400 border-yellow-400 scale-110' : 'border-gray-600'}`} />
                    ))}
                </div>
                {error && <p className="text-red-400 text-xs text-center mb-4 mt-[-1rem]">{error}</p>}
                <div className="grid grid-cols-3 gap-3">
                    {[1, 2, 3, 4, 5, 6, 7, 8, 9, '', 0, '⌫'].map((d, i) => (
                        <button key={i} onClick={() => d === '⌫' ? del() : d !== '' && handleDigit(String(d))}
                            className={`h-14 rounded-2xl font-bold text-xl transition-colors ${d === '' ? 'cursor-default' : 'bg-gray-800 text-white hover:bg-gray-700 active:bg-gray-600'}`}>
                            {d}
                        </button>
                    ))}
                </div>
